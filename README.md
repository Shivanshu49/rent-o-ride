...
| `app_role` holds **zero** table-level UPDATE | every writable column named in a `GRANT` | `grant-surface.e2e-spec.ts` reads `information_schema` back and asserts the whole surface in both directions: every table in `public` must be declared, and every grant must match the declaration. A table-level `GRANT UPDATE` silently covers a column added three phases later, so a regressing migration fails CI instead of passing |
| A new table does not inherit a write grant | `ALTER DEFAULT PRIVILEGES ... REVOKE UPDATE, DELETE` | a table created mid-test gives `app_role` only SELECT and INSERT; the same test checks the default was set by the role migrations actually run as |
| A DELETE grant is never dead weight | a DELETE policy behind every DELETE grant | `bookings`, `kyc_documents`, `owner_profiles`, `referrals` and `platform_settings` had DELETE with no policy; `trips` had one, which was worse — the row holds the handover photos and odometer readings that settle a damage dispute |
